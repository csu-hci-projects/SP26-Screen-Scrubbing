# SP26-Screen-Scrubbing

The purpose of this repository is to hold data and code for a VR video editing application.

### Team Members

Davis Garrett  
Connor Schwab  
Riley Bowling


## Links to Checkpoint 1 Videos
Current State of Project - https://youtu.be/4tA_kWONb9U \
Code Walkthrough - https://youtu.be/YUmo7FESMLw 

## Links to Checkpoint 2 Videos
Code Walkthrough - https://youtu.be/29ygqZ-pGrE \
Current State - https://youtu.be/663-TvwLvdQ 

## Links to Final Videos
Code Walkthrough - https://youtu.be/Q7SzQr270cc \
Project Walkthrough - https://youtu.be/7oGqZVZM3Gk \
Project Presentation - https://youtu.be/Ry91b-6hEbs 

## Project Paper

See the `/Documents` folder for our LaTeX paper file and for a PDF version of the paper.
The LaTeX file is named sample-sigconf.tex


## How To Run

- Unity 6 

- XR Interaction Toolkit installed
  
- Start with controllers disabled in HandTracking mode


## Controls

- **Hand Tracking** — hover and pinch to select clip to move, or to select handles to resize. Move hand to move clip or resize accordingly.

- **Controllers** — hover and use side trigger to select clip to move, or to select handles to resize. Move controller to move clip or resize accordingly.

